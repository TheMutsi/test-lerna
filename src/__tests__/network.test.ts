...
describe('Log network call With Axios', () => {
  test('Verify the instance has an Axios structure', () => {
    ...
    const { applyNetworkInterceptors } = require('../embrace');
    expect(applyNetworkInterceptors(axiosMockedOK)).toEqual(1);
    expect(applyNetworkInterceptors()).toEqual(0);
    expect(applyNetworkInterceptors(axiosMockedNOK)).toEqual(0);
    expect(applyNetworkInterceptors(axiosMockedNOKJustInterceptor)).toEqual(0);
    expect(
