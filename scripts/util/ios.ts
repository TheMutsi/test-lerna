...
export const appDelegatePatchable = ({
  ...
  return new Promise((resolve, reject) => {
    const appDelegatePath = glob
      .sync('**/AppDelegate.*(m|mm)', { ignore: 'node_modules/**' })
      .find(
        (path: string) =>
          path.toLocaleLowerCase().indexOf(name.toLocaleLowerCase()) > -1
      );
    if (!appDelegatePath) {
      return reject(
        embLogger.format(
          'Couldn\'t find AppDelegate. Please refer to the docs at https://docs.embrace.io to update manually.'
        )
      );
    }
