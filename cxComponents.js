...
ComponentStrategy.productAndFitHelp = function productAndFitHelp() {
  var message = 'I need help with product and fit';
  var title = 'Product and Fit Help';
  var subtitle = 'Sizing tips, product details & deals';
  var escapedMessage = message.replace(/'/g, "\\'").replace(/"/g, '\\"');
  var escapedTitle = title.replace(/'/g, "\\'").replace(/"/g, '\\"');
  return '<button class="component-button" data-component="productAndFitHelp" onclick="if(window.ChatWidget && window.ChatWidget.sendMessageThroughButton) { window.ChatWidget.sendMessageThroughButton(\'' + escapedMessage + '\', \'' + escapedTitle + '\'); }" style="background-color: #ffffff; color: #292929; border: none; padding: 12px 16px; cursor: pointer; border-radius: 15px; font-family: \'Avenir Next\', sans-serif; font-weight: 400; font-size: 16px; line-height: 24px; box-sizing: border-box; width: 270px; height: 68px; box-shadow: 0px 4px 10px 0px #CED2D980; transition: all 200ms ease; text-align: left; white-space: normal; word-wrap: break-word; overflow-wrap: break-word; display: flex; flex-direction: column; align-items: flex-start;"><span style="font-weight: 600; font-size: 16px; line-height: 24px; font-family: \'Avenir Next\', sans-serif; color: #292929; margin-bottom: 2px;">' + ComponentStrategy.escapeHtml(title) + '</span><span style="font-weight: 400; font-size: 14px; line-height: 18px; font-family: \'Avenir Next\', sans-serif; color: #6b7280;">' + ComponentStrategy.escapeHtml(subtitle) + '</span></button>';
};